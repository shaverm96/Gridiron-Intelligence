...
[cite_start]Gridiron Intelligence is a production-ready, full-stack LLM application built with Streamlit designed for college football scouting[cite: 6, 16]. It solves the problem of fragmented personnel evaluations by unifying historical data, live statistics, and unstructured web intelligence into a single platform. [cite_start]The application serves recruiting staff and analysts by supporting two core workflows: evaluating high school prospects and assessing college transfer impacts[cite: 7, 8]. [cite_start]To achieve this, the system implements a multi-agent orchestration graph featuring an explicit fan-out/fan-in topology[cite: 9]. By balancing deterministic data retrieval with dynamic, agentic web exploration, Gridiron Intelligence delivers consistent, traceable, and highly contextualized decision support for football personnel departments.
...
* [cite_start]Generating structured evaluations for high school recruiting prospects[cite: 7].
* [cite_start]Generating impact analyses for college transfer candidates[cite: 8].

[cite_start]The application must overcome the "agentic scale trap" [cite: 12] by preserving reliability and explainability while safely incorporating multi-agent reasoning, web scraping, and API tool usage into a production-grade interface.

### 1.3 Project Objectives

[cite_start]This project synthesizes core LLM concepts into a unified system[cite: 17, 19]:
* **Prompt Engineering:** Designing task-specific prompts to ensure consistent report formatting and grounded syntheses.
* [cite_start]**Tool Calling:** Integrating external data systems including the CFBD API, DuckDuckGo web search, and a Supabase backend[cite: 22].
* [cite_start]**Agent Architecture:** Implementing a modular, fan-out/fan-in graph to separate task delegation from final synthesis[cite: 9].
* [cite_start]**Security and Robustness:** Applying strict prompt budget caps, payload truncation, and explicit rate limiting to ensure production safety[cite: 12].
* [cite_start]**Performance Scaling:** Utilizing a fixed model cascade (FrugalGPT style) to optimize inference costs without sacrificing reasoning quality[cite: 11].

## 2. Methodology

### 2.1 System Overview

The system features a Landing Page that routes users into either the Recruiting Portal or the Transfer Portal. [cite_start]In the Recruiting Portal, the app generates evaluations using vector insights, web scouting, and historical comparables[cite: 7]. [cite_start]The Transfer Portal evaluates candidates utilizing CFBD API data alongside live web context[cite: 8]. Both structured workflows produce deterministic report artifacts, which then seed the context for a stateful, multi-agent follow-up chat.
...
5.  [cite_start]**External Systems:** The backend relies on Supabase for state/data, the CFBD API for college stats, DuckDuckGo for live news, and the Gemini 3.1 API for generative capabilities[cite: 10, 11].

### 2.3 Multi-Agent Workflow Design

[cite_start]Gridiron Intelligence utilizes a multi-agent orchestration model structured as an explicit fan-out/fan-in topology[cite: 9]. 
* [cite_start]**Delegation:** The `lead_delegator` node evaluates the user's intent and delegates tasks[cite: 9].
* [cite_start]**Parallel Workers:** Specialized workers, such as the `cfbd_analyst`, `recruiting_scout`, and `team_scout`, execute tasks concurrently[cite: 9].
* [cite_start]**Synthesis:** The `lead_synthesizer` acts as the fan-in aggregator, merging the evidence collected by the workers into a final, grounded response[cite: 9].
...
[cite_start]To guarantee safe production behavior and avoid the "agentic scale trap"[cite: 12], the system implements robust guardrails:
* **Validation:** Delegator outputs are strictly validated, explicitly halting execution if the requested plan is malformed.
* [cite_start]**Payload Management:** Prompt budget caps and payload truncation prevent out-of-memory errors and excessive token costs[cite: 12].
* **Sanitization:** Summaries are sanitized to strip HTML, scripts, and code blocks before being written to state.
* [cite_start]**Rate Limiting:** Strict submission rate limiters are applied to structured report generation to prevent system abuse[cite: 12].
...
* [cite_start]**CFBD API:** Provides canonical, live data pulls for college transfer candidates, including player usage metrics and season statistics[cite: 8].
...
[cite_start]The backend was recently refactored to incorporate a split-identity database model[cite: 10]. This explicit separation reduces identity ambiguity and streamlines multi-agent retrieval. The core tables include:
* [cite_start]`gi_recruit_master`: Identity and context for high school prospects[cite: 10].
* [cite_start]`gi_college_master`: Identity and context for college careers[cite: 10].
* [cite_start]`gi_player_link_bridge`: The crosswalk table linking recruit records to college/CFBD identities[cite: 10].

### 3.3 Data Characteristics and Quality

[cite_start]*(Insert table populated with dataset statistics based on `notebooks/supabase_upserts_pipeline.ipynb` outputs. Emphasize that identity resolution relies on SQL-first fuzzy matching (`pg_trgm`) rather than embeddings to ensure deterministic, high-accuracy lookups[cite: 10].)*
...
[cite_start]RAG is utilized in two specific ways: retrieving vector-based factoids for high school prospects, and using a SQL-first fuzzy matching algorithm (`pg_trgm`) for reliable entity resolution without the overhead of dense embeddings[cite: 10].
...
### 4.4 Agent Design
[cite_start]The system uses an explicit `lead_delegator` to plan execution, passing instructions to parallel workers (`cfbd_analyst`, `recruiting_scout`, `team_scout`), and feeding their outputs into a `lead_synthesizer`[cite: 9]. This prevents endless ReAct loops and ensures predictable execution paths.
...
[cite_start]To achieve cost efficiency at scale, the project implements a fixed model cascade (FrugalGPT style)[cite: 11]. [cite_start]Upstream web and API summarization tasks are routed to the highly efficient Gemini 3.1 Flash-Lite, reserving the heavier Gemini 3.1 Flash model exclusively for final synthesis[cite: 11].
